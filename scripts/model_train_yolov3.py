...
# os.environ["MXNET_MKLDNN_ENABLED"]="1"
...
gdata.COCODetection.CLASSES = ('guohui', )


def get_dataset_coco(args):
    train_dataset = gdata.COCODetection(root=args.data_dir, splits='train', use_crowd=False)
    val_dataset = gdata.COCODetection(root=args.data_dir, splits='test', skip_empty=False)
...
# def get_dataset_voc(args):
#     train_dataset = gdata.VOCDetection( root=args.data_dir,
#         # splits=[(2007, 'trainval'), (2012, 'trainval')])
#                         splits=[(2007, 'train')])
#     val_dataset = gdata.VOCDetection( root=args.data_dir,
#         splits=[(2007, 'test')])
...
def get_dataset_voc(args):
    train_dataset = utils.VOCDetection(root=args.data_dir, splits=[(2007, 'train')])
    val_dataset   = utils.VOCDetection(root=args.data_dir, splits=[(2007, 'test')])
    val_metric = VOC07MApMetric(iou_thresh=0.5, class_names=val_dataset.classes)
...
def get_dataloader(net, train_dataset, val_dataset, data_shape, batch_size, num_workers, args):
    ...
    if args.no_random_shape:
        ...
        print('using Dataloader directly.')
    else:
        ...
        train_loader = RandomTransformDataLoader(
            ...
            # shuffle=False, batchify_fn=batchify_fn, num_workers=num_workers)
    print('train dataloader load successfully ...')
    val_batchify_fn = Tuple(Stack(), Pad(pad_val=-1))
    val_loader = gluon.data.DataLoader(
        val_dataset.transform(YOLO3DefaultValTransform(width, height)),
        batch_size, False, batchify_fn=val_batchify_fn, last_batch='keep', num_workers=num_workers)
    print('test dataloader load successfully ...')
...
def train(net, train_data, val_data, eval_metric, ctx, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, batch in enumerate(train_data):
            ...
            # print('shape of gt_boxes: ', gt_boxes[0].shape)       # (1, 1, 4)
            ...
            with autograd.record():
                for ix, x in enumerate(data):
                    ...
                    obj_loss, box_loss, cls_loss = net(x, gt_boxes[ix], *[ft[ix] for ft in fixed_targets])
                    # print('shape of obj_loss: ', obj_loss.shape)
                    # print('shape of box_loss: ', box_loss.shape)
                    # print('shape of cls_loss: ', cls_loss.shape)
                    # print('obj_loss: ', obj_loss)
                    # print('box_loss: ', box_loss)
                    # print('cls_loss: ', cls_loss)
                    sum_losses.append(obj_loss + box_loss + cls_loss)
                    obj_losses.append(obj_loss)
                    center_losses.append(box_loss)
                    scale_losses.append(box_loss)
                    cls_losses.append(cls_loss)
...
def parse_args():
    ...
    parser.add_argument('--data-dir', type=str, default='./datasets/', help='the root dir of dataset')
    ...
    parser.add_argument('--class_nums', type=int, default=1, help='the class nums used by yolov3')
...
if __name__ == '__main__':
    ...
    # net_name = 'yolo3_mobilenet1.0_voc'
    net_name = 'yolo3_darknet53_custom'
    args.save_prefix += net_name
    # use sync bn if specified
    if args.syncbn and len(ctx) > 1:
        net = get_model(net_name, classes=classes, pretrained_base=True, norm_layer=gluon.contrib.nn.SyncBatchNorm,
                        norm_kwargs={'num_devices': len(ctx)})
        async_net = get_model(net_name, classes=classes, pretrained_base=False)  # used by cpu worker
    else:
        net = get_model(net_name, classes=classes, pretrained_base=True)
        async_net = net
    ...
    # training data
    # args.batch_size = 10
    # args.data_dir = './datasets/guohui_detect/refine2/'
    batch_size = args.batch_size
    # args.num_workers = 1
    # train_dataset, val_dataset, eval_metric = get_dataset_coco(args)
    train_dataset, val_dataset, eval_metric = get_dataset_voc(args)
    ...
    print('dataloader create successful.')
    # training
