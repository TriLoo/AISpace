...
class VOCMApMetric(mx.metric.EvalMetric):
    ...
    """
    def __init__(self, iou_thresh=0.5, class_names=None, score_thresh=0.0, roc_output_path=None):
    ...
    def save_roc_graph(self, recall=None, prec=None, score=None, classkey=1, path=None, ap=None):
        class_name = self.class_names[classkey]
        joblib_file = os.path.join(path, 'prec_recall_' + class_name + '.joblib')
        joblib.dump({'prec' : prec, 'recall' : recall, 'score' : score}, joblib_file)
    ...
    def _update(self):
        ...
        aps = []
        recall, precs, scores = self._recall_prec()
        for l, rec, prec, score in zip(range(len(precs)), recall, precs, scores):
            ap = self._average_precision(rec, prec)
            if self.roc_output_path is not None:
                if l == 1:
                    print('recall', rec[-1])
                    print('prec', prec[-1])
                self.save_roc_graph(rec, prec, score, l, self.roc_output_path, ap)
            aps.append(ap)
